...
- **Use on-device test pages for cross-browser calibration.** When Playwright/Chromium screenshots can't be trusted (e.g. hieroglyph positioning), deploy a test page with multiple variants (A/B/C/D) so the user can pick the best one on their actual device. The test page MUST use the exact same grid, padding, and all 12 categories so `grid-auto-rows: 1fr` produces real card dimensions. Example: `eye-test.html`. This is a valid precision tool — use it instead of guessing.
