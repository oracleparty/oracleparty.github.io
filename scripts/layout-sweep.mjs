...
function measure(stressText) {
  ...
  const out = { vw, overflow: [], scrollers: [], ragged: [], clipped: [], unstyled: [] };
...
console.log(`LAYOUT SWEEP — ${names.length} state(s) x ${WIDTHS.join('/')}px${STRESS ? '  [STRESS]' : ''}`);
console.log('='.repeat(72));
...
for (const name of names) {
  ...
  for (const width of WIDTHS) {
    ...
    try {
      ...
      const bad = r.overflow.length + r.scrollers.length + r.ragged.length;
      ...
      faults++;
      console.log(`\n✗ ${name} @ ${width}px — threw: ${e.message.split('\n')[0]}`);
    } finally {
      await context.close();
    }
  }
